...
void clearHexDigit(unsigned int& param) {
	...
	if(digitNumber < 0 || digitNumber > 7){
		cout << "Out of bounds. Please enter a digit between 0 and 7." << endl;
	}
	unsigned int mask = 0xF;
	mask = mask << 4*digitNumber;
	mask = ~mask;
	result = param & mask; 
...
void defineNewValueForHexDigit(unsigned int& param) {
	...
	mask1 = mask1 << 4*digitNumber;
	mask1 = ~mask1;
	result = param & mask1;
	unsigned int mask2 = newValue & 0xF; 
	mask2 = mask2 << 4*digitNumber;
	result = result | mask2; 
